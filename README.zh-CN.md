...
只临时关闭 `install.sh` 启动的飞书监听器与 Codex watcher，并保留 hooks、shell 函数、`.env` 和自启动配置：
...
`install.sh` 的启动范围包括飞书回调监听器和 Codex 交互卡监听器：
...
- 无 systemd 环境：使用 nohup 进程并写入 crontab `@reboot`

`stop-services.sh` / `npm run services:stop` 只停止这些常驻服务，不移除 hooks、shell 函数、`.env`、`node_modules` 或自启动配置。

```bash
bash install.sh          # 安装（自动清理旧配置 → 重新安装并启动服务）
bash stop-services.sh    # 停止常驻服务（保留安装配置）
bash uninstall.sh        # 卸载（停止服务 → 清理配置）
npm run services:stop    # 等价于 bash stop-services.sh
```
